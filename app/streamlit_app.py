...
# --- KPI dictionary (for quick orientation) ---
with st.expander("KPI dictionary (tap to expand)", expanded=False):
    st.markdown(
        """
        - **ts** – event timestamp (UTC)  
        - **cell_id** – unique identifier for the radio cell  
        - **anomaly_flag** – 1 if this row tripped the rule-based anomaly pre-filter; 0 otherwise  
        - **latency_ms** – mean user-plane latency (ms) for the interval  
        - **prb_util_pct** – PRB utilization (%); proxy for capacity load  
        - **thrpt_mbps** – downlink throughput (Mbps)  
        - **sinr_db** – Signal-to-Interference-plus-Noise Ratio (dB) — higher is better *(if present)*  
        - **rsrp_dbm** – Reference Signal Received Power (dBm) — less negative is better *(if present)*  
        - **pkt_loss_pct / drop_rate_pct** – packet loss (%) / call drop rate (%) *(if present)*  
        - **region / date** – partition columns for filtering and grouping
        """
    )
...
if scores_path.exists():
    ...
    # --- Explain the anomaly-rate table ---
    st.caption(
        "Anomaly rate = share of recent rows flagged by the pre-filter per cell. "
        "Use with **latency_ms_p95** and **prb_util_pct_avg** to tell if it’s "
        "**capacity** (high PRB), **radio quality** (low SINR / poor RSRP), or **transport** (loss/jitter)."
    )
    ...
    st.dataframe(
        ...
        column_config={
            ...
            "latency_ms_p95":   st.column_config.NumberColumn("Latency p95 (ms)", format="%.0f"),
            "prb_util_pct_avg": st.column_config.NumberColumn("PRB util (%)", format="%.1f"),
            "sinr_db_avg":      st.column_config.NumberColumn("SINR (dB)", format="%.1f"),
            "thrpt_mbps_avg":   st.column_config.NumberColumn("Downlink throughput (Mbps)", format="%.0f"),
        }
    )

    st.info("**Rule of thumb** — > **0.05**: take a look • > **0.10**: sustained; should appear in Alerts once risk crosses the operating threshold.")
    ...
    thr_model = float(thr)   # model’s operating threshold (best F1), e.g., ~0.09
    triage_default = 0.05    # start triage lower to surface more cells

    # --- Optional UI override to see more/fewer alerts ---
    thr = st.slider(
        "Alert threshold (override)",
        min_value=0.00, max_value=1.00, value=float(triage_default), step=0.01,
        help=(
            f"Default shown here ({triage_default:.2f}) is a triage cutoff to surface more cells. "
            f"The model’s operating threshold (best F1) from artifacts/metrics.json is {thr_model:.2f}."
        ),
    )
    if thr != thr_model:
        st.caption(f"Using triage threshold {thr:.2f} (model op thr = {thr_model:.2f}).")
    ...
    # choose existing columns only (keeps file integrity)
        ...
        # Download actionable alerts (after computing priority & columns)
        _dl_cols = (['priority'] + [c for c in cols if c != 'priority']) if 'priority' in alerts.columns else cols
        _sort_key = 'priority' if 'priority' in alerts.columns else 'risk_score'
        st.download_button(
            'Download actionable alerts (CSV)',
            alerts.sort_values(_sort_key, ascending=False)[_dl_cols].to_csv(index=False).encode(),
...
with colA:
    if _cent is not None:
        ...
        if not row.empty:
            ...
            # brief interpretation under the numbers (left column)
            st.caption(
                "How to read:\n\n"
                ...
                "**Betweenness** shows if the cell is a **bridge** on shortest paths (higher → issues can fragment clusters)."
            )
...
st.caption("Centrality indicates a cell’s **network influence**. Centrality features are consumed by the model and shown in the risk table; "
...
if fc_path.exists():
    ...
    st.caption(
        "Forecast shows **PRB utilization** (capacity load). "
        "**Line = median** forecast; **shaded band = uncertainty**. "
        "Use it to anticipate capacity pressure (e.g., crossing 85%)."
    )
   ...
   # --- Brief interpretation for the selected cell (auto) ---
    try:
        thr_prb = 85.0   # PRB capacity threshold
        horizon = 12     # hours to look ahead (adjust to your plotted window)

        # filter to the selected cell
        df_f_sel = fc[fc["cell_id"] == sel_cell].copy()
        df_future = df_f_sel[df_f_sel["ds"] > df_f_sel["ds"].max() - pd.Timedelta(hours=horizon)]

        # Probability of breach in horizon (upper band exceeding threshold)
        p_breach = (df_future["yhat_upper"] >= thr_prb).mean() if not df_future.empty else 0.0
        last_mid = float(df_f_sel["yhat"].tail(1)) if not df_f_sel.empty else float("nan")

        st.caption(
            f"**Reading for {sel_cell}**: median PRB ≈ **{last_mid:.0f}%**. "
            f"**{p_breach:.0%}** chance of crossing **{thr_prb:.0f}%** in the next **{horizon}h** "
            "(based on forecast upper band)."
        )

    except Exception as e:
        st.info(f"Interpretation unavailable (check forecast inputs): {e}")
...
st.caption(
    "Percent of cell-hours that met each target in the selected window: "
    "**Capacity** (PRB<85%), **Latency** (p95<60ms), **Reliability** (drop<0.5%). "
    "Use low SLOs to focus triage, then drill into Alerts, Forecast, and Centrality."
)
...
st.caption(
    ...
    "- **Latency SLO** (93.7%): Most traffic meets the 60ms target, but a few cells may drift.\n\n"
    "- **Reliability SLO** (28.2%): Packet drops are frequent — this is the weakest dimension.\n\n"
)
st.caption(
    "_Note: In a full deployment these SLOs recompute with the current filters—region and date window—so the summary reflects the exact slice you’re reviewing._"
)
...
if metrics_path.exists():
    ...
    st.caption(
    "**Interpretation:** \n "
    "AUC-ROC of 0.745 means the model separates risky vs. normal cells fairly well (75% probability a random risky cell scores higher than a normal one). \n"
    "AUC-PR of 0.241 reflects performance in imbalanced data—above baseline (~0.125) so it’s catching more true risks than random guessing. \n"
    "The operating threshold of 0.09 balances recall and precision: low enough to surface risky cells early but not so low that false positives dominate. \n "
    "F1 of 0.437 shows the trade-off—reasonable balance, though ops teams might tune this higher/lower depending on tolerance for false alarms."
)
...
st.caption(
    "Warehouse mart powering the metrics/features. "
    "Use this for auditing, validation, and BI export—same fields the model and alerts consume."
)
...
with st.sidebar:
    ...
    - **Graph analytics**: neighbor degree, PageRank, betweenness  
    - **Warehouse**: Postgres / DuckDB mirror + view  
    - **Triage logic**: priority = risk ⨉ forecast ⨉ graph  
    - **AI/ML stack**: NumPy, scikit-learn, XGBoost, Prophet  
    - **Data infra**: data lake / warehouse + NoSQL / graph DB exposure  
    - **4G/5G KPIs**: PRB, RSRP/SINR, latency, drops  
    - **Scoring at scale**: anomaly detection + monitoring  
