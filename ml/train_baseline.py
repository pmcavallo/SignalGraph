...
try:
    ...
    if explainer is not None:
        # Limit size for speed; compute on held-out test window
        ...
        # --- Top reasons per alert for the latest hour (if time keys exist) ---
        latest_ts = None
        ...
        if latest_ts is not None and {"cell_id","ts_hour"}.issubset(df.columns):
            latest_mask = df["ts_hour"] == latest_ts
            eval_latest = df.loc[latest_mask, features]
            if len(eval_latest):
                sv_latest = explainer.shap_values(eval_latest)
                if isinstance(sv_latest, list):
                    sv_latest = sv_latest[1] if len(sv_latest) > 1 else sv_latest[0]
                abs_sv = _np.abs(sv_latest)
                top_idx = abs_sv.argsort(axis=1)[:, -3:][:, ::-1]  # top-3 features
                top_names = _np.array(features)[top_idx]
                top_str = [", ".join(map(str, row)) for row in top_names]
                reasons = _pd.DataFrame({
                    "cell_id": df.loc[latest_mask, "cell_id"].values,
                    "ts_hour": df.loc[latest_mask, "ts_hour"].values,
                    "top_reasons": top_str
                })
                reasons.to_parquet(ART / "shap_top_reasons.parquet", index=False)
